...
import com.app.sistconApp.repository.CobrancaRepository;
...
public class CobrancaServiceImpl implements CobrancaService {
	...
	@Autowired
	private CobrancaRepository cobr;
	...
	public Page<Cobranca> listarPagina(Pageable pagina) {
		...
		return cobr.findAllByCondominioOrderByDataEmissaoDescMoradiaAscNumeroAscParcelaAsc(condominio, pagina);
	}
	...
	public void validar(Cobranca entidade, BindingResult validacao) {
		...
		if (entidade.getIdCobranca() == null) {
			if (entidade.getDataEmissao() != null && entidade.getMoradia() != null
					&& cobr.existsByNumeroAndParcelaAndDataEmissaoAndMoradiaAndCondominio(entidade.getNumero(),
		...
		// VALIDAÇÕES NA ALTERAÇÃO
			...
			if (entidade.getDataEmissao() != null && entidade.getMoradia() != null
					&& cobr.existsByNumeroAndParcelaAndDataEmissaoAndMoradiaAndCondominioAndIdCobrancaNot(
	...
	public BigDecimal inadimplencia() {
		...
		if (condominio == null) {
			...
			resultado = cobr.sumTotalByCondominioAndDataVencimentoBeforeAndDataRecebimentoIsNull(condominio,
					LocalDate.now());
			if (resultado == null) {
				resultado = BigDecimal.ZERO.setScale(2);
			}
